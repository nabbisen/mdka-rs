...
cells! {
    strikethrough_like_text: "<p>~~not struck~~</p>"
        ...
        defect(Rfc010Planned, "GFM reading: `~~` is not escaped, so the text becomes strikethrough and the tildes are lost (CommonMark reading correct)");
    ...
    table_like_lines_with_pipes: "<p>| a | b |<br>| --- | --- |</p>"
        => tree(r#"para("| a | b |", br, "| --- | --- |")"#),
        defect(Rfc010Planned, "GFM reading: two text lines with `|` become a table; the delimiter row is consumed and the pipes lost (CommonMark reading correct)");
    table_like_lines_with_alignment: "<p>a | b<br>:-- | --:</p>"
        => tree(r#"para("a | b", br, ":-- | --:")"#),
        defect(Rfc010Planned, "GFM reading: a `:--` line is not escaped, so the lines become a table and the delimiter row is lost (CommonMark reading correct)");
