...
cells! {
    fence_content_with_triple_backticks: "<pre><code>a\n```\nb</code></pre>"
        => tree(r#"codeblock("a\n```\nb")"#),
        defect(Rfc010Planned, "fence fixed at three backticks: the content's ``` closes it early and a stray fence is left open (audit A-05)");
    fence_content_with_four_backticks: "<pre><code>````</code></pre>"
        => tree(r#"codeblock("````")"#),
        defect(Rfc010Planned, "fence fixed at three backticks: content ```` is read as a fence; the block is emptied and one left open (audit A-05)");
    ...
    code_span_with_backtick: "<p><code>a`b</code></p>"
        ...
        defect(Rfc010Planned, "backtick in a code span is backslash-escaped instead of widening the delimiter: the span ends early (audits A-03, A-05)");
    code_span_with_double_backtick: "<p><code>a``b</code></p>"
        => tree(r#"para(code("a``b"))"#),
        defect(Rfc010Planned, "backticks in a code span are backslash-escaped instead of widening the delimiter: the span ends early (audits A-03, A-05)");
    code_span_starting_with_backtick: "<p><code>`a</code></p>"
        => tree(r#"para(code("`a"))"#),
        defect(Rfc010Planned, "leading backtick in a code span is backslash-escaped: the span ends after the backslash (audits A-03, A-05)");
}
...
cells! {
    link_destination_with_space: r#"<a href="/a b.html">x</a>"#
        ...
        defect(Rfc010Planned, "destination with a space emitted bare: not a link at all, literal `[x](/a b.html)` (audit A-04)");
    ...
    link_destination_with_unbalanced_paren: r#"<a href="/a)b">x</a>"#
        ...
        defect(Rfc010Planned, "unbalanced `)` in the destination is not escaped: the link ends early at `/a` (audit A-04)");
    ...
    link_title_with_quotes: r#"<a href="/x" title='say "hi"'>x</a>"#
        ...
        defect(Rfc010Planned, "a double quote in the title is not escaped: not a link at all (audit A-04)");
    image_source_with_space: r#"<img src="a b.png" alt="x">"#
        => tree(r#"para(image[a b.png]("x"))"#),
        defect(Rfc010Planned, "source with a space emitted bare: not an image at all (audit A-04)");
    image_title_with_quotes: r#"<img src="i.png" alt="x" title='a "b"'>"#
        => tree(r#"para(image[i.png "a \"b\""]("x"))"#),
        defect(Rfc010Planned, "a double quote in the title is not escaped: not an image at all (audit A-04)");
}
