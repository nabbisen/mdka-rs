...
`Balanced` and `Minimal` write exactly what `2.9.0` wrote, asserted by a test that
converts a corpus of 58 documents in both modes and compares every byte with
goldens generated by running the published `2.9.0`. So there is nothing to
re-verify and no output to diff: everything below is **compile-and-rename**.

**Read the [migration guide](https://nabbisen.github.io/mdka-rs/migration-3.0.html)**
(`docs/src/migration-3.0.md`) — it gives every break per binding with before and
after, and a search list for finding your call sites. This entry only lets you
recognise whether you are affected.

**Who is affected.** If you only call the string functions (`html_to_markdown`,
`html_to_markdown_with`, `html_to_markdown_many` and their per-language forms) with
`Balanced`, `Minimal` or no mode, **nothing changes for you.** **Node.js callers are
touched most:** the file functions returned one object that served both single and
bulk conversion, so every Node.js caller that converts a file is affected, and Node.js
was the only binding holding two conventions at once (`htmlToMarkdownWith` beside
`htmlToMarkdownMany(htmls, options?)`), so four function names go.

This is the first breaking release since `2.0.0`, and the first `mdka` has staged
deliberately: `2.0.0` was a rewrite reconstructed after the fact, with no runway and
no guide. `2.9.0` changed nothing and warned about the last removal, and `3.0.0`
followed it immediately.

### Removed

**All bindings.**
- **Modes `Strict`, `Semantic` and `Preserve`** — aliases of `Balanced`, byte-for-byte
  the same output. Use `Balanced`. A mode read from a *string* now fails with
  `conversion mode 'strict' was removed in 3.0; it was an alias of 'balanced'. Use
  'balanced'.` — deliberately different from `unknown conversion mode`, since a
  removed name is an obsolete configuration, not a wrong one. **Change the strings in
  your config files now:** nothing else will find them.
- **The options that never did anything:** `preserve_classes`, `preserve_data_attrs`
  and `preserve_aria_attrs` (Rust, Node.js, Python, CLI), and, **Rust only**,
  `preserve_unknown_attrs`, `drop_presentation_attrs` and the builder
  `ConversionOptions::preserve_aria_attrs`. Markdown has no attribute syntax, so they
  never changed the output. Delete them.
- **`unwrap_unknown_wrappers`** (`unwrapUnknownWrappers`, `--unwrap-wrappers`) —
  **written for a reader who never saw its warning**, because it was deprecated in
  `2.9.0` and removed in `3.0.0`, which followed immediately. It was an option that
  controlled whether a wrapper element (`<div>`, `<section>`, `<article>`, `<main>`)
  had its tag removed. It **could not change the output**: unwrapping removes the tag
  but keeps the paragraph break it stood for, and Markdown has no wrapper element to
  show the difference. Removing the call changes nothing. `Minimal` still unwraps
  wrappers and `Balanced` still renders them, as a property of the mode. If wrapper
  handling ever becomes expressible it returns as a **new** option; this one does not
  come back.

**Rust.**
- **`ConvertResult`.** `html_file_to_markdown` and `html_file_to_markdown_with` now
  return `Result<PathBuf, MdkaError>` — the destination path. The caller already had
  the input path.
- **The tuple return.** `html_files_to_markdown` and `html_files_to_markdown_with` now
  return `Vec<FileOutcome>` (`FileOutcome { src: PathBuf, result: Result<PathBuf,
  MdkaError> }`) instead of `Vec<(&P, Result<PathBuf, MdkaError>)>`, and lose their
  `'a` lifetime parameter: an outcome owns its `src`. **`FileOutcome` is not `Clone`**
  (`MdkaError` wraps `std::io::Error`), so a caller that cloned parts of the old tuple
  must restructure — the one break that is not a rename.

**Node.js.**
- **`ConvertResult`** — gone. `htmlFileToMarkdown` now resolves to the destination
  path, a `string`, and a failure **rejects**; `htmlFilesToMarkdown` resolves to
  `Array<FileOutcome>` (`{ src, dest?, error?, ok }`) and a failing file does not
  reject the call.
- **`htmlToMarkdownWith`, `htmlToMarkdownWithAsync`, `htmlFileToMarkdownWith` and
  `htmlFilesToMarkdownWith`** — gone. Options are now an optional last argument on the
  base names: `htmlToMarkdown(html, options?)`, `htmlToMarkdownAsync(html, options?)`,
  `htmlFileToMarkdown(path, outDir?, options?)`, `htmlFilesToMarkdown(paths, outDir,
  options?)`. `htmlToMarkdownMany` and `version` are unchanged. Rust and Python keep
  their `_with` pairs, which are their idiom.
- The type `ConvertResult` and the option keys `preserveClasses`, `preserveDataAttrs`,
  `preserveAriaAttrs` and `unwrapUnknownWrappers`.

**Python.**
- **`ConvertResult`** — gone. `html_file_to_markdown` and `html_file_to_markdown_with`
  return the destination path as a `str`, and still raise `MdkaError` on failure.
- **`BulkConvertResult` is renamed `FileOutcome`**, with the same four fields (`src`,
  `dest`, `error`, `ok`); it is frozen. Its `repr` is now Python's own, showing all
  four fields — it printed `error=Some("…")`, a Rust `Option`, and omitted `dest` and
  `ok`.
- `ConversionMode.Strict`, `Semantic`, `Preserve` and the keyword arguments
  `preserve_classes`, `preserve_data_attrs`, `preserve_aria_attrs` and
  `unwrap_unknown_wrappers`.

**CLI.** `--mode strict|semantic|preserve`, `--preserve-classes`, `--preserve-data`,
`--preserve-aria` and `--unwrap-wrappers` now exit with status 1 and one line on
stderr saying what to do — for example ``error: `--unwrap-wrappers` was removed in
3.0; it could not change the output. Remove it from the command line: the output is
the same without it.`` — so a script that still passes one fails loudly rather than
looking as though it worked.

### Changed

- **Node.js now rejects an unrecognised option key, naming it.** napi used to drop any
  key it did not know, so `{ preserveClasses: true }` — or a typo like `{ mdoe:
  'minimal' }` — silently did nothing. It is now an error (`option 'preserveClasses'
  was removed in 3.0; …`, `unknown option 'mdoe'. Valid options: mode, preserveIds,
  dropInteractiveShell`): thrown by the synchronous functions, rejected by the
  Promise-returning ones. **This is a behaviour change for code that was already
  wrong**, and it means a call that passed a removed option and worked before will now
  fail.
- **Node.js `FileOutcome` has `ok`**, which Node.js did not have; exactly one of `dest`
  and `error` is set and `ok` says which.
- **`ConversionMode::parse_mode` is deprecated** (since `3.0.0`), not removed. It
  discards the reason a name was rejected, so a removed mode name and an unknown one
  both give `None`. Use `str::parse`, which reports why.

### Documentation

- **The [migration guide](https://nabbisen.github.io/mdka-rs/migration-3.0.html)** is
  new.
- The API reference now covers all three bindings side by side, and the architecture
  page lists `src/table.rs`. **The performance page now says its figures describe
  `2.3.0` and have not been re-measured since.**
...
[RFC 048](https://github.com/nabbisen/mdka-rs/blob/main/rfcs/done/048-the-3.0-surface.md)
for its shape.
