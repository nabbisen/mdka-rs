...
There were five until 3.0. `Strict`, `Semantic` and `Preserve` were aliases of
`Balanced` that produced identical output and could not differ — what would have
distinguished them is attribute handling, and Markdown has no attribute syntax —
