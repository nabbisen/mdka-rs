...
# update_napi_binding_js <path> <old_version> <new_version>
#
# node/index.js is generated by napi-rs and stamps the crate version into its
# native-binding version checks, both as a quoted constant and inside an error
# message template. version.sh never knew about this file, and the post-update
# assertion below only re-checks files it touched - so the drift was invisible
# by construction.
#
# It drifted for six releases: at 2.1.8 the generated loader still said 2.0.2,
# and shipped to npm that way. Any consumer setting NAPI_RS_ENFORCE_VERSION_CHECK
# got "expected 2.0.2 but got 2.1.8" on load (roadmap R-02). RFC 006 corrected it
# by regenerating, but a regeneration only helps if someone remembers to run one
# - the very next bump put it back out of step. This makes the bump carry it.
#
# The crate version is the ONLY version-like string in the file, so a global
# substitution is equivalent to regenerating. Verified at the 2.2.0 bump: all 52
# matches were the package version and no other semver string appeared.
update_napi_binding_js() {
    file_path=$1; old=$2; new=$3
    [ ! -f "$file_path" ] && return
    grep -qF "$old" "$file_path" || return

    count=$(grep -oF "$old" "$file_path" | wc -l | tr -d ' ')

    if [ "$DRY_RUN" -eq 1 ]; then
        printf '  (dry-run) would update %s (%s occurrences)\n' "$file_path" "$count"
        return
    fi

    old_escaped=$(printf '%s' "$old" | sed 's/[.[\*^$/]/\\&/g')
    tmp=$(mktemp) || exit 1
    sed "s/$old_escaped/$new/g" "$file_path" > "$tmp"
    mv "$tmp" "$file_path"
    git add "$file_path"
    printf '  updated %s (%s occurrences)\n' "$file_path" "$count"

    # Own assertion: the shared one below matches "\"$OLD_VERSION\"" with double
    # quotes, which would never match this file's single-quoted JS constants.
    if grep -qF "$old" "$file_path"; then
...
if [ "$UPDATE_MODE" -eq 1 ]; then
    ...
    # 4b. napi-rs が生成する node/index.js のバインディング版数チェック文字列
    #     (see update_napi_binding_js comment above)
    update_napi_binding_js "$WORKSPACE_ROOT/node/index.js" "$OLD_VERSION" "$NEW_VERSION"
