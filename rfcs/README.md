...
| ID | Title | State |
|----|-------|-------|
| 048 | [The `3.0` surface: one result model, one convention per language, two modes](./accepted/048-the-3.0-surface.md)  — [`2.9.0` precondition](./handoffs/048-the-3.0-surface/2.9.0-precondition-handoff.md) · [`2.9.0` API reference](./handoffs/048-the-3.0-surface/2.9.0-api-reference-handoff.md) · [slice 1: removals](./handoffs/048-the-3.0-surface/slice-1-removals-handoff.md) | **Accepted (owner, 2026-09-25); §11 answered the same day.** Resolves RFC 039 Half B and RFC 041 §5.1 together. **No conversion output changes.** `3.0` ships **immediately after `2.9.0`**, which is a precondition, not an option — §7. **The `2.9.0` precondition ships in `2.9.0`** — `unwrap_unknown_wrappers` deprecated on all four surfaces, the string-form behaviour pinned, and the API reference made tri-lingual; **everything else is `3.0`.** Stays in `accepted/` |
